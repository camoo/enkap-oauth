...
$query = $this->Payments->query()->set(['status' => $status])->where(['oder_transaction_id' => $trxId]);
if ($status === StatusService::CONFIRMED_STATUS){
    // Payment successfully completed
    // send Item to user/customer
    return;
}

if (in_array($status, [StatusService::FAILED_STATUS, StatusService::CANCELED_STATUS])) {
...
$setup = new SetupService($key, $secret);
...
$setup->setCallbackUrls($callBack);
```
